...
Image duplicateImage(Image img) {
	...
	for (int i = 0; i < temp->width * temp->height * 3; i++) {
...
unsigned char* imageData(Image img) {
  return img->data;
}
...
void imageFillRectRgb(Image img, int left, int top, int right, int bottom, Rgb rgb) {
	imageFillRect(img, left, top, right, bottom, rgb.red, rgb.green, rgb.blue);
}

void imageFillRect(Image img, int left, int top, int right, int bottom, int red, int green, int blue) {
